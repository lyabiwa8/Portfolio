...
export function ModernBackground() {
  ...
  useEffect(() => {
    ...
    const loadVanta = async () => {
      try {
        // Load p5 first as Vanta depends on it
        ...
        if (vantaRef.current && !vantaEffect) {
          effect = TOPOLOGY({
            ...
            scale: 1.00,
            scaleMobile: 1.00,
            color: 0x2f3b24, // Deep green lines
  ...
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden bg-[#F2F4E8]">
      {/* ─── Vanta Topology Overlay (Subtle) ─── */}
      <div 
        ref={vantaRef} 
        className="absolute inset-0 z-0 opacity-[0.2] mix-blend-multiply pointer-events-none" 
      />

      {/* ─── Framer-Style Mesh Gradients (Light Green Version) ─── */}
      <div className="absolute inset-0 filter blur-[140px] opacity-70 z-[-1]">
        <motion.div
          animate={{
            x: ["-15%", "15%", "-10%", "10%", "-15%"],
            y: ["-10%", "20%", "5%", "-15%", "-10%"],
